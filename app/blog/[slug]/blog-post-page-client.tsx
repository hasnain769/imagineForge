...
export default function BlogPostPageClient({ postData }: { postData: { title: string, date: string, contentHtml: string, image: string } }) {
  return (
    <section className="min-h-screen w-full py-24 px-4 md:px-8 bg-transparent">
      <div className="max-w-4xl mx-auto z-10 pt-16">
        <AnimatedH1>{postData.title}</AnimatedH1>
        <AnimatedDiv
        ...
        >
          <Image src={postData.image} alt={`A decorative image for the blog post titled: ${postData.title}`} layout="fill" objectFit="cover" />
        </AnimatedDiv>
        <AnimatedDiv
          initial={{ opacity: 0, y: 30 }}
          animate={{ opacity: 1, y: 0 }}
